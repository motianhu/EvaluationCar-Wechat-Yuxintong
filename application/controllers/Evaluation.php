...
//         	$res = array('status'=>'0','msg'=>$type.'第'.$no.'张图片文件格式错误.'.$filename);
//         	echo json_encode($res);
// 			exit;
//         }
		if( ! in_array(exif_imagetype($_FILES['image_file']['tmp_name']),array(IMAGETYPE_GIF,IMAGETYPE_JPEG,IMAGETYPE_PNG))){
			$res = array('status'=>'0','msg'=>$type.'第'.$no.'张图片文件格式错误.'.$filename);
        	echo json_encode($res);
			exit;
		}
